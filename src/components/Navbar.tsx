...
export default function Navbar() {
  ...
  return (
    <>
      ...
      <div className="absolute top-4 right-4 z-50">
        ...
        >
          <div className="relative w-6 h-6">
            <span
              className={`absolute top-2.5 left-0 w-6 h-0.5 ${colors.glass.text} bg-current rounded-full transition-all duration-300 ${isMenuOpen ? "rotate-45" : "rotate-0 -translate-y-1.5"}`}
            ></span>
            <span
              className={`absolute top-2.5 left-0 w-6 h-0.5 ${colors.glass.text} bg-current rounded-full transition-all duration-300 ${isMenuOpen ? "opacity-0" : "opacity-100"}`}
            ></span>
            <span
              className={`absolute top-2.5 left-0 w-6 h-0.5 ${colors.glass.text} bg-current rounded-full transition-all duration-300 ${isMenuOpen ? "-rotate-45" : "rotate-0 translate-y-1.5"}`}
            ></span>
      ...
      {/* Slide-out Menu */}
      <nav
        className={`fixed top-0 right-0 h-full w-80 ${colors.glass.bg} backdrop-blur-xl border-l ${colors.glass.border} shadow-2xl transform transition-transform duration-300 ease-in-out z-[9999] ${
        ...
        <div className="flex flex-col h-full pt-20 px-6">
          ...
          <div className="flex-1 flex flex-col space-y-2">
            {navItems.map((item: NavItem) => {
              ...
              const isDropdownActive = item.subItems?.some(
                (subItem: NavSubItem) => pathname === subItem.href,
              );
              ...
              if (item.dropdown && item.subItems) {
                return (
                  <div key={item.href} className="space-y-1">
                    ...
                    >
                      <span
                        className={`${fontCombinations.navigation.primary} transition-colors duration-50 ease-out ${
                          isDropdownActive || openDropdown === item.label
                      ...
                      <div className="ml-4 space-y-1">
                        {item.subItems.map((subItem: NavSubItem) => {
                          ...
                          return (
                            ...
                            >
                              <span
                                className={`${fontCombinations.navigation.secondary} transition-colors duration-50 ease-out ${
                                  isSubActive
                                    ? colors.nav.textActive
                                    : `${colors.glass.text} group-hover:${colors.nav.textActive}`
                                }`}
          ...
          <div className="border-t border-white/20 pt-6 pb-8">
            <div className="text-center">
              <p
                className={`${colors.glass.textSubtle} ${fontCombinations.content.small} ${hierarchyWeights.footer}`}
              >
                ALPHA KAPPA PSI
              </p>
              <p
