    ...
    >
      ...
      <div className="max-w-6xl mx-auto px-6 py-6 md:py-8">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-8 mb-4 md:mb-6 items-start justify-between mt-4">
          ...
          <div className="space-y-2 md:space-y-3 mb-4 md:mb-0 flex flex-col items-center md:items-start">
            <h3
              className={`text-3xl ${colors.text.inverse} ${akpsiFonts.sectionTitleFont} ${hierarchyWeights.footer} mb-6`}
            >
              Quick Links
            </h3>
            ...
            <div className="space-y-3 md:space-y-3 flex flex-col items-center md:items-start">
              <div className="space-y-2 md:space-y-1 flex flex-col items-center md:items-start">
                {rushInfo.chairs.map((rushChair) => (
                  ...
                  >
                    <p
                      className={`text-sm ${colors.text.inverse} opacity-60 ${akpsiFonts.bodyFont}`}
                    >
                      {rushChair.name}:
                    </p>
                    <a
                ...
                <p
                  className={`text-sm ${colors.text.inverse} ${akpsiFonts.bodyFont}`}
                >
                  <a
                    href={`mailto:${rushInfo.email}`}
                    className={`text-sm ${colors.text.inverse} opacity-70 hover:opacity-100 transition-opacity ${colors.footer.link} ${akpsiFonts.bodyFont}`}
                  >
                    <u>{rushInfo.email}</u>
                  </a>
                </p>
          ...
          <div className="flex items-center space-x-6">
            {socialIcons.map((social) => (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className={`${colors.footer.icon} transition-colors`}
      ...
      <div className="border-t border-[#9E9E9E]/30 px-6 py-4 copyright-section">
        <div className="max-w-6xl mx-auto flex flex-col items-center justify-center space-y-0">
          <p
            className={`text-sm ${colors.text.inverse} opacity-60 ${akpsiFonts.bodyFont} text-center whitespace-nowrap`}
          >
            &copy; {new Date().getFullYear()} UCSD Alpha Kappa Psi. All rights
            reserved.
          </p>
          <p
            className={`text-sm ${colors.text.inverse} opacity-60 ${akpsiFonts.bodyFont} flex items-center`}
          >
            <a
              href="/terms"
              className={`text-sm ${colors.text.inverse} opacity-70 hover:opacity-100 transition-opacity ${colors.footer.link} ${akpsiFonts.bodyFont}`}
