...
const backgroundImage = "/rush/fall25Background.png";
const flyerImage = "/rush/fall25Flyer.png";
        ...
        <main className="flex-1 flex items-center justify-center py-16 px-4 mt-20">
          ...
          {!loading && (
            <BouncyFadeIn
              ...
              bounce={0}
              threshold={0}
              className={`${colors.glass.bg} backdrop-blur-md rounded-2xl shadow-2xl p-10 max-w-6xl w-full mx-auto ${colors.glass.border}`}
            >
              <div>
                ...
                <div className="grid lg:grid-cols-2 gap-12 mb-12">
                  ...
                  <div className="space-y-6">
                    <h2
                      className={`text-4xl ${fontCombinations.section.main} ${rushColors.rushTextHighlight}`}
                    >
                      ABOUT RECRUITMENT
                    </h2>
                    <p
                      className={`text-lg leading-relaxed ${rushColors.rushTextBody} ${fontCombinations.content.body}`}
                    >
                      The brothers of Alpha Kappa Psi cordially invite you to
                      attend our Fall 2026 In-Person Rush:
                      <span className={rushColors.rushTextHighlightBold}>
                        {" "}
                    ...
                    <p
                      className={`text-lg leading-relaxed ${rushColors.rushTextBody} ${fontCombinations.content.body}`}
                    >
                      Come out in Week 2 of Fall Quarter to meet our amazing
                      active members and esteemed alumni. Get a glimpse of the
                      professional development and tight-knit community we can
                      offer.
                    </p>
                    <p
                      className={`text-lg ${fontCombinations.navigation.secondary} ${rushColors.rushTextHighlight} ${fontCombinations.content.lead}`}
                    >
                      Please fill out the interest form to stay updated on our
                      upcoming events!
                    </p>
                ...
                <div className="mb-12">
                  <h2
                    className={`text-4xl ${fontCombinations.section.main} mb-8 text-center ${rushColors.rushText}`}
                  >
                    RUSH WEEK SCHEDULE
                  </h2>
                  <div className="space-y-6">
                    {loading ? (
                      <div
                        className={`${colors.glass.bg} backdrop-blur-sm rounded-lg p-6 ${colors.glass.border} text-center`}
                      >
                        <p
                          className={`${rushColors.rushText} ${fontCombinations.content.body}`}
                        >
                          Loading events...
                        </p>
                      </div>
                      ...
                      recruitmentEvents.map((event, index) => (
                        <div
                          key={index}
                          className={`${colors.glass.bg} backdrop-blur-sm rounded-lg p-6 ${colors.glass.border}`}
                        >
                          <h3
                            className={`text-xl ${fontCombinations.navigation.primary} mb-2 ${rushColors.rushTextHighlight}`}
                          >
                            {event.eventName.toUpperCase()}
                          </h3>
                          <p
                            className={`text-lg font-bold ${rushColors.rushTextHighlight} ${fontCombinations.navigation.primary} mb-2`}
                          >
                            {event.day.toUpperCase()}, {event.date}
                          </p>
                          <p
                            className={`text-lg font-medium ${rushColors.rushTextSubtitle} ${fontCombinations.navigation.secondary} mb-2 ${fontCombinations.content.small}`}
                          >
                            {event.details}
                          </p>
                          <p
                            className={`${rushColors.rushTextBody} ${fontCombinations.content.body}`}
                          >
                            {event.description}
                          </p>
                        </div>
                ...
                <div className="text-center">
                  <h2
                    className={`text-4xl ${fontCombinations.section.main} mb-6 ${rushColors.rushText}`}
                  >
                    QUESTIONS? CONTACT US!
                  </h2>
                  <div
                    className={`${colors.glass.bg} backdrop-blur-sm rounded-lg p-8 ${colors.glass.border}`}
                  >
                    <h3
                      className={`text-xl ${fontCombinations.section.tertiary} mb-4 ${rushColors.rushTextHighlight}`}
                    >
                      Fall &apos;26 Rush
                    </h3>
                    <div className={`space-y-2 ${rushColors.rushTextBody}`}>
                      {contactInfo.chairs.map((chair, index) => (
                        <p
                          key={index}
                          className={`text-lg ${fontCombinations.content.lead} break-words block`}
                        >
                          <span className="font-semibold">{chair.name}:</span>{" "}
                          <span className="break-all">{chair.number}</span>
                        </p>
                      ))}
                      <div
                        className={`text-lg mt-4 ${fontCombinations.content.lead} flex flex-col md:flex-row items-center justify-center gap-2 md:gap-0 text-center flex-wrap`}
                      >
